...
@section('content')
    <div class="w-full mx-auto" x-data="{ 
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden mb-6">
            ...
            {{-- Form Filter --}}
            <form action="{{ route('komandan.tamu') }}" method="GET" class="p-4 border-b border-gray-200"
                x-data="{ cari: '{{ request('cari') }}' }">
                <div class="flex flex-wrap gap-4">
                    ...
                    <div class="w-[calc(50%-0.5rem)] md:flex-1">
                        ...
                        <div class="cursor-pointer" @click="$refs.dateEnd.showPicker()">
                            <input type="date" id="end_date" name="end_date" x-ref="dateEnd" onchange="this.form.submit()"
                                class="block w-full h-[42px] px-4 bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#1e3a5f] focus:border-[#1e3a5f] shadow-sm cursor-pointer"
                                value="{{ $endDate }}">
                        </div>
                    </div>
                ...
                <table class="w-full min-w-max table-fixed">
                    ...
                    <tbody class="text-sm divide-y divide-gray-200">
                        @forelse($riwayatTamu as $index => $tamu)
                            <tr>
                                <td class="py-2 px-4 text-center">{{ $riwayatTamu->firstItem() + $index }}.</td>
                                <td class="py-2 px-4 font-medium">{{ $tamu->nama_tamu }}</td>
                                <td class="py-2 px-4 text-center">{{ $tamu->instansi }}</td>
                                <td class="py-2 px-4 text-center text-gray-700 whitespace-nowrap">
                                    {{ $tamu->waktu_datang->format('d/m/Y H:i') }}
                                </td>
                                <td class="py-2 px-4 text-center">{{ $tamu->tujuan }}</td>
                                @if(Auth::user()->peran == 'komandan')
                                    <td class="py-2 px-4">
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ Auth::user()->peran == 'komandan' ? '6' : '5' }}"
            ...
            @if($riwayatTamu->total() > 0)
                <div class="hidden md:flex justify-between items-center px-6 py-4 border-t border-gray-200">
                    <div class="text-sm text-gray-600">
                        Showing {{ $riwayatTamu->firstItem() ?? 0 }} to {{ $riwayatTamu->lastItem() ?? 0 }} of
                        {{ $riwayatTamu->total() }} entries
                    </div>
                    <div class="flex gap-1">
                        ...
                        @else
                            <a href="{{ $riwayatTamu->appends(['per_page' => request('per_page'), 'start_date' => request('start_date'), 'end_date' => request('end_date')])->previousPageUrl() }}"
                        ...
                        @foreach($riwayatTamu->getUrlRange(1, $riwayatTamu->lastPage()) as $page => $url)
                            ...
                            @else
                                <a href="{{ $riwayatTamu->appends(['per_page' => request('per_page'), 'start_date' => request('start_date'), 'end_date' => request('end_date')])->url($page) }}"
                        ...
                        @if($riwayatTamu->hasMorePages())
                            <a href="{{ $riwayatTamu->appends(['per_page' => request('per_page'), 'start_date' => request('start_date'), 'end_date' => request('end_date')])->nextPageUrl() }}"
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="p-4 bg-blue-50/50 rounded-xl border border-blue-100">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div>
                                        <label for="nama_tamu"
                                            class="block text-xs font-bold text-[#1e3a5f] uppercase tracking-wide mb-1">Nama
                                            Tamu <span class="text-red-500">*</span></label>
                                        <div class="relative">
                                            <div
                                            ...
                                            <input type="text" id="nama_tamu" name="nama_tamu" x-model="editNama" required
                                    ...
                                    <div>
                                        <label for="instansi"
                                            class="block text-xs font-bold text-[#1e3a5f] uppercase tracking-wide mb-1">Instansi
                                        ...
                                        <div class="relative">
                                            ...
                                            <input type="text" id="instansi" name="instansi" x-model="editInstansi" required
                                    ...
                                    <div>
                                        <label for="tujuan"
                                            class="block text-xs font-bold text-[#1e3a5f] uppercase tracking-wide mb-1">Tujuan
                                            <span class="text-red-500">*</span></label>
                                        <div class="relative">
                                            <div
                                                class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            ...
                                            <input type="text" id="tujuan" name="tujuan" x-model="editTujuan" required
                                    ...
                                    <div>
                                        ...
                                        <div class="relative">
                                            ...
                                            <input type="datetime-local" id="waktu_datang" name="waktu_datang"
                                                x-model="editWaktuDatang" :max="maxDate" required
                                                class="pl-10 w-full bg-white border border-gray-300 text-gray-800 text-sm font-medium rounded-lg shadow-sm focus:ring-[#1e3a5f] focus:border-[#1e3a5f] block p-2.5">
                                        </div>
                                    </div>
                                </div>
