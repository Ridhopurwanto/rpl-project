...
<div class="w-full mx-auto"
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden mb-6" x-data="{ showTemuan: true }">
        ...
        {{-- Form Filter Temuan --}}
        <form action="{{ route('bau.barang.index') }}" method="GET" class="p-4 border-b border-gray-200" x-data="{}">
            <input type="hidden" name="tanggal_titipan" value="{{ $tanggalTitipan }}">
            <input type="hidden" name="search_titipan" value="{{ $searchTitipan }}">
            <input type="hidden" name="per_page_titipan" value="{{ $perPageTitipan }}">
            <div class="flex flex-wrap gap-4">
                ...
                <div class="w-[calc(50%-0.5rem)] md:flex-1">
                    ...
                    <div class="cursor-pointer" @click="$refs.dateInputTemuan.showPicker()">
                        <input type="date" id="tanggal_temuan" name="tanggal_temuan" x-ref="dateInputTemuan" onchange="this.form.submit()" class="block w-full h-[42px] px-4 bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#1e3a5f] focus:border-[#1e3a5f] shadow-sm cursor-pointer" value="{{ $tanggalTemuan }}">
                    </div>
                </div>
        ...
        <div class="hidden md:flex justify-between items-center px-6 py-4 border-t border-gray-200">
            ...
            <div class="flex gap-1">
                ...
                @else
                    <a href="{{ $barangTemuan->appends(request()->except('page_temuan'))->previousPageUrl() }}" class="px-3 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">Previous</a>
                @endif
                @foreach($barangTemuan->getUrlRange(1, $barangTemuan->lastPage()) as $page => $url)
                    ...
                    @else
                        <a href="{{ $barangTemuan->appends(request()->except('page_temuan'))->url($page) }}" class="px-3 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">{{ $page }}</a>
                    @endif
                @endforeach
                @if($barangTemuan->hasMorePages())
                    <a href="{{ $barangTemuan->appends(request()->except('page_temuan'))->nextPageUrl() }}" class="px-3 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">Next</a>
                @else
        ...
        <div class="md:hidden flex justify-between items-center px-3 py-4 border-t border-gray-200">
            ...
            <div class="flex gap-1">
                ...
                @else
                    <a href="{{ $barangTemuan->appends(request()->except('page_temuan'))->previousPageUrl() }}" class="px-2 py-1 text-xs text-gray-700 bg-white border border-gray-300 rounded hover:bg-gray-50">Prev</a>
                @endif
                @foreach($barangTemuan->getUrlRange(1, $barangTemuan->lastPage()) as $page => $url)
                    @if($page == $barangTemuan->currentPage())
                        <span class="px-2 py-1 text-xs text-white bg-[#1e3a5f] rounded">{{ $page }}</span>
                    @else
                        <a href="{{ $barangTemuan->appends(request()->except('page_temuan'))->url($page) }}" class="px-2 py-1 text-xs text-gray-700 bg-white border border-gray-300 rounded hover:bg-gray-50">{{ $page }}</a>
                    @endif
                @endforeach
                @if($barangTemuan->hasMorePages())
                    <a href="{{ $barangTemuan->appends(request()->except('page_temuan'))->nextPageUrl() }}" class="px-2 py-1 text-xs text-gray-700 bg-white border border-gray-300 rounded hover:bg-gray-50">Next</a>
                @else
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden mb-6" x-data="{ showTitipan: true }">
        ...
        <form action="{{ route('bau.barang.index') }}" method="GET" class="p-4 border-b border-gray-200" x-data="{}">
            ...
            <input type="hidden" name="search_temuan" value="{{ $searchTemuan }}">
            <input type="hidden" name="per_page_temuan" value="{{ $perPageTemuan }}">
            <div class="flex flex-wrap gap-4">
                <div class="w-[calc(50%-0.5rem)] md:w-auto">
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">Show</label>
                    <div class="flex items-center gap-2">
                        <select name="per_page_titipan" onchange="this.form.submit()" class="h-[42px] pl-4 pr-10 bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#1e3a5f] focus:border-[#1e3a5f] shadow-sm appearance-none cursor-pointer" style="background-image: url('data:image/svg+xml;charset=UTF-8,%3csvg xmlns=%27http://www.w3.org/2000/svg%27 viewBox=%270 0 24 24%27 fill=%27none%27 stroke=%27currentColor%27 stroke-width=%272%27 stroke-linecap=%27round%27 stroke-linejoin=%27round%27%3e%3cpolyline points=%276 9 12 15 18 9%27%3e%3c/polyline%3e%3c/svg%3e'); background-repeat: no-repeat; background-position: right 0.75rem center; background-size: 1.25em 1.25em;">
                ...
                <div class="w-[calc(50%-0.5rem)] md:flex-1">
                    ...
                    <div class="cursor-pointer" @click="$refs.dateInputTitipan.showPicker()">
                        <input type="date" id="tanggal_titipan" name="tanggal_titipan" x-ref="dateInputTitipan" onchange="this.form.submit()" class="block w-full h-[42px] px-4 bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#1e3a5f] focus:border-[#1e3a5f] shadow-sm cursor-pointer" value="{{ $tanggalTitipan }}">
                    </div>
                </div>
                <div class="w-full md:flex-1">
        ...
        <div class="hidden md:flex justify-between items-center px-6 py-4 border-t border-gray-200">
            ...
            <div class="flex gap-1">
                @if($barangTitipan->onFirstPage())
                    <span class="px-3 py-2 text-sm text-gray-400 bg-gray-100 rounded-lg cursor-not-allowed">Previous</span>
                @else
                    <a href="{{ $barangTitipan->appends(request()->except('page_titipan'))->previousPageUrl() }}" class="px-3 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">Previous</a>
                @endif
                @foreach($barangTitipan->getUrlRange(1, $barangTitipan->lastPage()) as $page => $url)
                    @if($page == $barangTitipan->currentPage())
                        <span class="px-3 py-2 text-sm text-white bg-[#1e3a5f] rounded-lg">{{ $page }}</span>
                    @else
                        <a href="{{ $barangTitipan->appends(request()->except('page_titipan'))->url($page) }}" class="px-3 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">{{ $page }}</a>
                    @endif
                @endforeach
                @if($barangTitipan->hasMorePages())
                    <a href="{{ $barangTitipan->appends(request()->except('page_titipan'))->nextPageUrl() }}" class="px-3 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">Next</a>
                @else
                    <span class="px-3 py-2 text-sm text-gray-400 bg-gray-100 rounded-lg cursor-not-allowed">Next</span>
                @endif
            </div>
        ...
        <div class="md:hidden flex justify-between items-center px-3 py-4 border-t border-gray-200">
            ...
            <div class="flex gap-1">
                @if($barangTitipan->onFirstPage())
                    <span class="px-2 py-1 text-xs text-gray-400 bg-gray-100 rounded cursor-not-allowed">Prev</span>
                @else
                    <a href="{{ $barangTitipan->appends(request()->except('page_titipan'))->previousPageUrl() }}" class="px-2 py-1 text-xs text-gray-700 bg-white border border-gray-300 rounded hover:bg-gray-50">Prev</a>
                @endif
                @foreach($barangTitipan->getUrlRange(1, $barangTitipan->lastPage()) as $page => $url)
                    @if($page == $barangTitipan->currentPage())
                        <span class="px-2 py-1 text-xs text-white bg-[#1e3a5f] rounded">{{ $page }}</span>
                    @else
                        <a href="{{ $barangTitipan->appends(request()->except('page_titipan'))->url($page) }}" class="px-2 py-1 text-xs text-gray-700 bg-white border border-gray-300 rounded hover:bg-gray-50">{{ $page }}</a>
                    @endif
                @endforeach
                @if($barangTitipan->hasMorePages())
                    <a href="{{ $barangTitipan->appends(request()->except('page_titipan'))->nextPageUrl() }}" class="px-2 py-1 text-xs text-gray-700 bg-white border border-gray-300 rounded hover:bg-gray-50">Next</a>
                @else
                    <span class="px-2 py-1 text-xs text-gray-400 bg-gray-100 rounded cursor-not-allowed">Next</span>
                @endif
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchInputTemuan.addEventListener('input', function() {
    ...
    searchInputTitipan.addEventListener('input', function() {
        clearTimeout(timeout);
        timeout = setTimeout(() => {
            this.closest('form').submit();
        }, 500);
