...
{{-- 
  WRAPPER UTAMA
  - Menggunakan Grid, di mobile (default) hanya 1 kolom.
  - Di 'lg' (desktop), akan menjadi 2 kolom (grid-cols-2)
--}}
<div class="min-h-screen grid lg:grid-cols-2">
    ...
    <div class="hidden lg:flex flex-col items-center justify-center p-12 text-center
        ...
        <div class="relative w-64 h-64 bg-white rounded-full 
            ...
            <img src="{{ asset('images/logo-siap.png') }}" alt="Logo SIAP" class="w-32 h-32 z-10">
        </div>
        
        <h1 class="text-white text-7xl font-bold mt-12 tracking-wide">SIAP</h1>
        <p class="text-gray-300 text-lg mt-4 tracking-wider max-w-md">
            Sistem Informasi Administrasi dan Pelaporan
        </p>
    </div>


    <div class="min-h-screen bg-white lg:flex lg:items-center lg:justify-center lg:bg-white">

        {{-- CARD UTAMA (KODE ASLI ANDA) --}}
        <div class="w-full bg-white lg:max-w-sm lg:mx-auto lg:shadow-2xl lg:rounded-3xl lg:overflow-hidden">

            {{-- 
              Header Card (Tampilan Mobile)
              - Di desktop (lg), bagian ini akan disembunyikan 
              - karena branding sudah ada di kolom kiri.
            --}}
            <div class="bg-gradient-to-b from-[#1a2847] via-[#243a5e] to-[#2a4a6f] pt-0 pb-12 text-white text-center relative lg:hidden">
                
                {{-- Elemen putih 'shield' (Bentuk perisai di Mobile) --}}
            ...
            {{-- 
              Bagian Form Login 
              - Di desktop (lg), margin-top negatif (-mt-6) dihapus (lg:mt-0)
              - dan rounded-top dihapus (lg:rounded-t-none)
            --}}
            <div class="bg-white px-8 py-10 -mt-6 rounded-t-[2.5rem] relative z-10 lg:mt-0 lg:rounded-t-none">
            ...
            <div class="mt-auto pt-10 pb-4 text-center text-gray-400 text-xs">
                &copy; {{ date('Y') }} SIAP. All rights reserved.
            </div>
        </div> 
    </div>
    </div>
