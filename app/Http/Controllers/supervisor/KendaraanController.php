...
class KendaraanController extends Controller
{
    ...
    public function editMaster($id_kendaraan)
    {
        if (Auth::user()->peran !== 'supervisor') {
            return redirect()->route('supervisor.kendaraan.index')->with('error', 'Anda tidak memiliki hak akses.');
        }

        try {
            $kendaraan = Kendaraan::findOrFail($id_kendaraan);
            
            // --- PERBAIKAN: Mengarah ke folder 'supervisor' ---
            return view('supervisor.kendaraan_edit', ['kendaraan' => $kendaraan]);
        
        } catch (\Exception $e) {
            return redirect()->route('supervisor.kendaraan.index')->with('error', 'Kendaraan tidak ditemukan.');
        }
    }
    ...
    public function updateMaster(Request $request, $id_kendaraan)
    {
        ...
        try {
            ...
            return redirect()->route('supervisor.kendaraan.index')->with('success', 'Data kendaraan berhasil diperbarui.');
        } catch (\Exception $e) {
    ...
    {
        // Keamanan: Hanya supervisor yang boleh
        if (Auth::user()->peran !== 'supervisor') {
            return redirect()->route('supervisor.kendaraan.index')->with('error', 'Anda tidak memiliki hak akses.');
        }
